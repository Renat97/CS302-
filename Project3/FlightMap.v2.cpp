...
///  problem 12 in book, adding try-next pointers into the adjacency list to begin the search where it left off instead of going back to the beginning
...
private:
    ...
    list<ItemType> *adj;
    /// an array which saves the last processed city in the adjacency list, this is the special implementation in FlightMap.v2, where the position in the list is saved and continued from there to see the next city, saves time from having to go back to the first index of the list.
